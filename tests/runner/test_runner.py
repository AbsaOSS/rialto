...
def test_load_module(basic_runner):
...
def test_completion(mocker, basic_runner):
    mocker.patch("rialto.runner.utils.table_exists", return_value=True)
    mocker.patch(
        "rialto.runner.utils.get_available_dates",
        return_value=[
            DateManager.str_to_date("2023-03-05"),
            DateManager.str_to_date("2023-03-12"),
            DateManager.str_to_date("2023-03-19"),
        ],
    )
    ...
    comp = basic_runner._get_completion(Table(table_path="catalog.schema.simple_group", partitions="DATE"), dates)
    expected = [False, True, True, True, False]
    assert comp == expected


def test_completion_rerun(spark, mocker):
    mocker.patch("rialto.runner.runner.utils.table_exists", return_value=True)
    mocker.patch(
        "rialto.runner.runner.utils.get_available_dates",
        return_value=[
            DateManager.str_to_date("2023-03-05"),
            DateManager.str_to_date("2023-03-12"),
            DateManager.str_to_date("2023-03-19"),
    ...
    comp = runner._get_completion(Table(table_path="catalog.schema.simple_group", partitions="DATE"), dates)
    expected = [False, True, True, True, False]
    assert comp == expected
...
def test_check_dates_have_partition(spark, mocker):
    ...
    dates = [DateManager.str_to_date(d) for d in dates]
    res = runner.has_data_for_dates(Table(schema_path="source.schema", table="dep1", partitions="DATE"), dates)
...
def test_check_dates_have_partition_no_table(spark, mocker):
    ...
    dates = ["2023-03-04", "2023-03-05", "2023-03-06"]
    dates = [DateManager.str_to_date(d) for d in dates]
    res = runner.has_data_for_dates(Table(schema_path="source.schema", table="dep66", partitions="DATE"), dates)
...
def test_select_dates(spark, mocker):
    ...
    r, i = runner._select_run_dates(
        runner.config.pipelines[0], Table(table_path="catalog.schema.simple_group", partitions="DATE")
    )
...
def test_select_dates_all_done(spark, mocker):
    ...
    mocker.patch(
        "rialto.runner.runner.utils.get_available_dates",
        return_value=[
            DateManager.str_to_date("2023-03-02"),
        ],
    )
