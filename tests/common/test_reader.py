...
def test_get_table_with_multiple_filters(multi_partition_df, mocker):
    mock_spark = mocker.MagicMock()
    mock_spark.read.table.return_value = multi_partition_df

    tr = TableReader(mock_spark)
    result = tr.get_table("test_table", date_column="info_date", filters={"region": "REGION_A", "type": "TYPE_X"})

    assert result.count() == 2

    assert result.first().info_date == date(2023, 1, 1)
    assert result.first().region == "REGION_A"
    assert result.first().type == "TYPE_X"
    assert result.first().value == 100


def test_get_table_without_filters(multi_partition_df, mocker):
    mock_spark = mocker.MagicMock()
    mock_spark.read.table.return_value = multi_partition_df

    tr = TableReader(mock_spark)
    result = tr.get_table("test_table", date_column="info_date")
