...
#             table.get_table_path(), date_column=table.partition,
#             date_from=info_date, date_to=info_date, filters=filters
#         )
#
#         return df.count()
#
...
#         :return: list of bool
#         """
#         if self.reader.table_exists(self.spark, table.get_table_path()):
#             checks = []
...
#             dep_from = date_manager.date_subtract(run_date, dependency.interval.units, dependency.interval.value)
#             logger.info(f"Looking for {dependency.table} from {dep_from} until {run_date}")
#
#             possible_dep_dates = date_manager.all_dates(dep_from, run_date)
#
...
#         """
#         possible_run_dates = date_manager.execution_dates(pipeline.schedule)
#         possible_info_dates = [DateManager.to_partition_date(x, pipeline.schedule) for x in possible_run_dates]
#         current_state = self._get_completion(table, possible_info_dates, filters)
#
#         selection = [
#             (run, info) for run, info, state in zip(possible_run_dates,
#             possible_info_dates, current_state) if not state
#         ]
#
